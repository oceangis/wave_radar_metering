...
_SENSOR_TYPES = {
    'elev': elev, 'velx': velx, 'vely': vely, 'velz': velz,
    'vels': vels, 'pres': pres, 'accs': accs, 'accz': accz,
    'slpx': slpx, 'slpy': slpy,
}

_METHODS = {
    'IMLM': IMLM, 'EMEP': EMEP, 'BDM': BDM, 'DFTM': DFTM, 'EMLM': EMLM,
}
...
def dirspec(ID, SM, EP, Options_=None):
    ...
    for m in range(szd):
        ...
        trm[m, :, :] = sensor_fn(2 * np.pi * F, pidirs, wns,
    ...
    method_fn = _METHODS[EP['method']]
    SM1['S'] = method_fn(xps[:, :, ffs], trm[:, ffs, :],
